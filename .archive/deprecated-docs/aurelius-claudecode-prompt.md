...
Class McpManager:
  ...
  async init():
    Connect to minimax-mcp-js (official JS MCP — image gen, video, TTS, music):
      ...
      const jsClient = new Client({ name: "aurelius-backend", version: "1.0.0" })
    ...
    Connect to minimax-coding-plan-mcp (web search + image understanding):
      ...
      const planClient = new Client({ name: "aurelius-backend", version: "1.0.0" })
...
Include:
1. What Aurelius is (2 sentences)
2. Prerequisites:
   - Bun (https://bun.sh)
...
After building, verify before moving to Android:
...
- [ ] POST /chat { messages: [{ role: "user", content: "Search the web for MiniMax AI news" }] }
      logs a web_search tool call and returns results in the response
...
Generate a complete Android project. Kotlin, minSdk 26, targetSdk 34.
The app connects to the backend and renders a streaming chat UI.
...
class ChatRepository {
  private val client = OkHttpClient.Builder()
    .readTimeout(120, TimeUnit.SECONDS)   // image/video gen can be slow
    .build()
...
2. RecyclerView (id: rvMessages) fills remaining space, background #0A0A0A,
   clipToPadding false, paddingBottom 8dp
3. Bottom input row (background #141414, top 1dp border #2A2A2A, 56dp tall):
   ...
   - ImageButton (id: btnSend): send icon, tint #4ADE80, 48×48dp,
     disabled tint #333333

## res/layout/item_message.xml

USER bubble:
  MaterialCardView, cardBackgroundColor #1C1C1C, strokeColor #2A2A2A, strokeWidth 1dp,
  cornerRadius 0dp, layout_gravity end, maxWidth 75% screen width,
  marginEnd 12dp, marginBottom 8dp
  Contains: TextView (text color #E8E8E8, 14sp, padding 10dp 8dp)

ASSISTANT bubble:
  Plain layout, layout_gravity start, maxWidth 85% screen width,
  marginStart 12dp, marginBottom 8dp
  Contains: TextView (text color #E8E8E8, 14sp)
  When isStreaming=true: append blinking cursor "▌" via Handler toggling every 500ms.
  When isStreaming=false: show plain text, stop animation.

## MessageAdapter.kt

RecyclerView.Adapter with two ViewTypes: USER (0) and ASSISTANT (1).
Use DiffUtil for efficient updates.
On ASSISTANT items where isStreaming=true, start the cursor blink in onBindViewHolder.
On ASSISTANT items where isStreaming=false, stop the blink and show plain text.

## MainActivity.kt

- Inflate with ViewBinding
- Set up RecyclerView with LinearLayoutManager, scroll to bottom on messages update
- Observe viewModel.messages: update adapter, scroll to last item on new content
- Observe viewModel.isLoading: toggle send button enabled state
- Send button click: grab etInput.text.toString(), call viewModel.sendMessage(), clear input
- Handle soft keyboard: scroll RecyclerView to bottom when input gains focus
- INTERNET permission in AndroidManifest.xml

## AndroidManifest.xml

  <uses-permission android:name="android.permission.INTERNET" />
  Single activity: MainActivity, exported=true, intent-filter MAIN/LAUNCHER.
...
- [ ] bun run dev starts — MCP server connections logged
- [ ] Tool list printed on startup (shows web_search, generate_image, text_to_audio, etc.)
- [ ] GET /health → { status: "ok" }
- [ ] Plain chat message streams and ends with [DONE]
...
Android:
- [ ] Gradle sync passes in Android Studio without errors
- [ ] assembleDebug builds without errors
- [ ] App launches on emulator — dark Aurelius chat UI visible
- [ ] Message sends, user bubble appears immediately
- [ ] Assistant bubble streams tokens live with blinking cursor
- [ ] Cursor disappears and text finalises on completion
- [ ] Conversation history maintained across turns
- [ ] Send button disabled during active stream
- [ ] readTimeout 120s set — handles slow image/video generation
